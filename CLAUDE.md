...
- `src/shared/token-resolver.service.ts` — `TOKEN_MAP` mapeia `BaseOrigin` → **nomes de variáveis de ambiente** (SGA, Lógica, Softruck, M7, Clubgas, Alloyal). Ao adicionar qualquer integração nova, é obrigatório preencher a entrada nas **duas** bases.
...
`SharedModule` é `@Global()` mas **não** é importado no `AppModule` — entra na aplicação via `RastreamentoModule`. Se esse import sumir, a DI quebra em toda a aplicação.

### Módulos globais

`DatabaseModule` (PrismaService), `InfraModule` (MailService + HealthController), `StorageModule` (StorageService → `LocalStorageProvider`), `QueueModule` (BullMQ/Redis), `SharedModule`. Alguns módulos ainda declaram `PrismaService`/`FileUploadService` nos próprios `providers` — resquício de migração incremental, não é conflito.
...
Três provedores externos: **M7**, **Lógica Soluções** e **Softruck**, cada um com subpasta própria (`m7/`, `logica/`, `softruck/`) organizada em `controllers/ services/ dto/ helpers/ mappers/ pdf/`. `RastreamentoService.rastreamento()` consulta os provedores em paralelo com `Promise.allSettled` e escolhe a posição mais recente.
...
- Pastas de DTO aparecem como `DTOs/` e `dto/` conforme o módulo — seguir o que já existe no módulo tocado.
