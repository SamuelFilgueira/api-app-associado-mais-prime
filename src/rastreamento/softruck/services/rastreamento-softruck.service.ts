...
import { UltimaPosicaoSoftruckResponse } from 'src/rastreamento/softruck/dto/ultima-posicao.dto';
import {
  CacheEntry,
  JwtPayload,
  ...
  TokenEntry,
} from 'src/rastreamento/softruck/interfaces/softruck-api.interface';
import { mapearUltimaPosicaoSoftruck } from 'src/rastreamento/softruck/mappers/ultima-posicao.mapper';
import {
  SoftruckByKeysApiResponse,
  SoftruckGeomFeatureCollection,
} from 'src/rastreamento/softruck/interfaces/softruck-trajectories.interface';
...
export class RastreamentoSoftruck {
  ...
  private async executeAutenticarSoftruck(
    ...
    try {
      ...
      this.logger.log(
        `[${baseOrigin}] Token Softruck atualizado (token=${maskSecret(token)}, exp=${new Date(expiresAt).toISOString()})`,
      );
    } catch (error) {
      if (axios.isAxiosError(error)) {
        this.logger.error(
