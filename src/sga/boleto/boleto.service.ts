...
export class BoletoService {
  async consultarBoletosPorVeiculo(codigo_veiculo: number) {
    ...
    dataInicial.setDate(now.getDate() - 6);
    const dataFinal = new Date(now);
    dataFinal.setDate(now.getDate() + 30);
    ...
    try {
      ...
      if (response.status === 200 && Array.isArray(response.data)) {
        for (const boleto of response.data) {
          boletos.push({
            ...
            placa: boleto.veiculos && boleto.veiculos[0] ? boleto.veiculos[0].placa : undefined
          });
        }
      } else {
      ...
      throw new InternalServerErrorException('Erro ao consultar boletos na Hinova');
    }
  }
}
