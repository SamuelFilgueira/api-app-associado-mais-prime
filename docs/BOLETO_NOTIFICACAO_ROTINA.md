...
- **Mais recente por placa**: havendo mais de um boleto EM ABERTO do mesmo tipo para a mesma placa, apenas o de `data_vencimento` mais recente participa da régua (os antigos são ignorados por completo, inclusive no catch-up). Para viabilizar essa regra, a rotina faz **uma única consulta ampla ao SGA por tenant/dia** (janela D-5 futuro até D+20+alcance no passado) e roteia as etapas em memória — falha nessa consulta marca todas as etapas do tenant como FALHA.
- **Regra global de faixas (catch-up)**: as etapas pós-vencimento cobrem FAIXAS de atraso, não só o dia exato — D+1: 1–4, D+5: 5, D+6: 6–19, D+20: 20 a 20+ (default 40). Um boleto ABERTO que entra atrasado na régua recebe a mensagem da faixa atual (uma única vez, via idempotência) e depois escala normalmente; boletos acompanhados desde o início continuam recebendo cada etapa no dia exato.
